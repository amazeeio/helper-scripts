...
echo "configmaps persistentvolumeclaims secrets services deployments.apps statefulsets horizontalpodautoscalers.autoscaling poddisruptionbudgets cronjobs.batch certificates.cert-manager.io ingresses.extensions mariadbconsumers.mariadb.amazee.io mongodbconsumers.mongodb.amazee.io postgresqlconsumers.postgres.amazee.io rolebindings.rbac.authorization.k8s.io roles.rbac.authorization.k8s.io" | xargs -n 1 \
  kubectl --context="$SOURCE" get -n "$NAMESPACE" -o json > "$NAMESPACE-original.json"

SOURCE_NAMESPACE_PVS=$(kubectl --context="$DESTINATION" get pv -o=json | jq ".items[]|select(.spec.claimRef.namespace==\"$NAMESPACE\")|.metadata.name" -r)
